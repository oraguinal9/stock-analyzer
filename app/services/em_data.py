        ...
        try:
            ...
            with httpx.Client(timeout=self.timeout, follow_redirects=True) as client:
                ...
                if data.get("data"):
                    ...
                    # 映射字段（东方财富数据单位：分，需要除以 100 转换为元）
                    # 东方财富不返回名称，需要从代码推断或使用腾讯 API
                    # 简单处理：使用代码作为名称
                    # 东方财富数据已经是元，不需要除以 100
                    stock = {
                        "code": code,
                        "name": f"{code}",
                        "price": d.get("f43", 0) if d.get("f43") else 0,
                        "change_pct": d.get("f170", 0) if d.get("f170") else 0,
                        "change_amount": d.get("f169", 0) if d.get("f169") else 0,
                        "market_cap": d.get("f116", 0),
                        "float_market_cap": d.get("f117", 0),
                        "pe": d.get("f164", 0) if d.get("f164") else 20.0,
                        "pb": d.get("f167", 0) if d.get("f167") else 3.0,
                        "roe": d.get("f168", 0) if d.get("f168") else 10.0,
                        "turnover": d.get("f166", 0) if d.get("f166") else 2.0,
                        "volume_ratio": d.get("f165", 0) if d.get("f165") else 1.0,
                    }
